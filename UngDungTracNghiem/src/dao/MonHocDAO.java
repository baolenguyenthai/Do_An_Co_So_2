...
public class MonHocDAO {
    ...
    public boolean insert(MonHoc m) {
        if (getIdByTen(m.getTenMonHoc()) != -1) {
    ...
    public boolean update(MonHoc m) {
        int existingId = getIdByTen(m.getTenMonHoc());
        if (existingId != -1 && existingId != m.getMonHocId()) {
            return false; // Tên đã được dùng bởi môn học khác
        }
    ...
    public int getIdByTen(String tenMonHoc) {
        String sql = "SELECT mon_hoc_id FROM mon_hoc WHERE LOWER(ten_mon_hoc) = LOWER(?)";
        try (Connection conn = KetNoiDB.ketNoi();
                PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public int themMonHoc(String tenMonHoc) {
        int existingId = getIdByTen(tenMonHoc);
        if (existingId != -1) {
            return existingId; // Trả về ID hiện có nếu đã tồn tại
        }
        String sql = "INSERT INTO mon_hoc(ten_mon_hoc) VALUES (?)";
        try (Connection conn = KetNoiDB.ketNoi();
