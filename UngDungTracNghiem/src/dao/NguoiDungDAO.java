...
public class NguoiDungDAO {
    ...
    public static String guiOTP(String email, String otp) {
        final String emailGui = layThongTinSMTP(
                "APP_OTP_EMAIL",
                "app.otp.email",
                "lenguyenthaib@gmail.com"
        );
        ...
        if (emailGui.isBlank() || matKhau.isBlank()) {
            return "Thiếu cấu hình SMTP. Hãy đặt APP_OTP_EMAIL và APP_OTP_PASSWORD.";
        }
        ...
        try {
            ...
            e.printStackTrace();
            return "Không thể đăng nhập Gmail SMTP (535). Vui lòng kiểm tra App Password 16 ký tự hoặc tạo App Password mới.";
        } catch (MessagingException e) {
            e.printStackTrace();
            return "Gửi OTP thất bại. Kiểm tra kết nối mạng hoặc cấu hình SMTP.";
        } catch (Exception e) {
            e.printStackTrace();
            return "Có lỗi không xác định khi gửi OTP.";
        }
    }

    private static String layThongTinSMTP(String envKey, String propKey, String fallback) {
        String tuEnv = System.getenv(envKey);
        if (tuEnv != null && !tuEnv.trim().isEmpty()) {
            return tuEnv.trim();
        }

        String tuProp = System.getProperty(propKey);
        if (tuProp != null && !tuProp.trim().isEmpty()) {
            return tuProp.trim();
        }
